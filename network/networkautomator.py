...
class NetworkAutomator:
        ...
        if dvs_selection == "1":
            if not is_multisystem_vds:
                ...
                system_dvs_to_pgs, pg_names_to_transport_types, vds_to_usedbynsxt_flag = \
                    self.input_multisystem_dvs_info(dvs_selection, pg_types_to_vmnics, dvpg_is_on, input_mgmt_pg_name,
            ...
            if not is_multisystem_vds:
                ...
                dvs_payload = self.prepare_dvs_payload_for_advanced_profile_multisystem(system_dvs_to_pgs,
                                                                                        ...
                                                                                        vmnics)
        print(*self.two_line_separator, sep='\n')
        self.utils.printCyan("(Multi NSX VDS) Please choose the DVS for which NSX VLAN needs to be tagged (optional):")
        self.utils.printBold("-----id-----vds-----")
        self.utils.printBold("----------------------")
        for i, dvs in enumerate(dvs_payload):
            self.utils.printBold("{}) {}"
                                 .format(i + 1, dvs['name']))
        dvs_options = input("\033[1m Enter your choices(comma separated if multiple / Empty): \033[0m")
        if dvs_options:
            dvs_options = list(map(int, dvs_options.strip().rstrip(",")
                                 .split(',')))
            for i, dvs in enumerate(dvs_payload):
                if 'isUsedByNsxt' in dvs.keys():
                    if dvs['isUsedByNsxt']:
                        dvs['nsxtSwitchConfig'] = {
                            'transportZones': [ {
                                'transportType': self.OVERLAY
                            }
                            ]
                        }
                    del dvs['isUsedByNsxt']
                if i+1 in dvs_options:
                    if not 'nsxtSwitchConfig' in dvs.keys():
                        dvs['nsxtSwitchConfig'] = {
                            'transportZones': []
                        }
                    dvs['nsxtSwitchConfig']['transportZones'].append({
                        'transportType': self.VLAN
                    })
        ...
        if nic_profile == 'ADVANCED_VXRAIL_SUPPLIED_VDS':
            if pg_type_to_active_uplinks is not None:
                ...
                if "VXRAILSYSTEMVM" in pg_type_to_active_uplinks:
                    portgroups.append(self.to_portgroup_obj_advanced(system_vm_pg_name, "VM_MANAGEMENT",
                ...
                portgroups = [self.to_portgroup_obj(mgmt_pg_name, "MANAGEMENT"),
                              ...
                              self.to_portgroup_obj(system_vm_pg_name, "VM_MANAGEMENT")]
                if vsan_pg_name is not None:
    ...
    def input_multisystem_dvs_info(self, dvs_selection, pg_types_to_vmnics, dvpg, input_mgmt_pg_name=None,
                ...
                for pg_type in pg_types_list:
                    if pg_type == 'MANAGEMENT':
                        ...
                        if pg_type == 'VSAN' and vsan_storage:
                            ...
                            if dvpg:
                                vm_management_pg_name = self.utils.valid_input(
                                    "\033[1m Enter portgroup name for type {} (vCenter Server Network-{}): \033[0m".format("VM_MANAGEMENT", cluster_name),
                                    "vCenter Server Network-" + cluster_name, self.utils.valid_resource_name) \
                                    if input_vm_management_pg_name is None else input_vm_management_pg_name
                            else:
                                vm_management_pg_name = "vCenter Server Network-" + cluster_name
                            portgroups.append(vm_management_pg_name)
                            pg_names_to_transport_types[vm_management_pg_name] = pg_type
