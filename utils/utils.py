...
class Utils:
    ...
    def poll_on_queries_for_remote_datastores(self, url):
        response = self.get_request(url)
        status = response['queryInfo']['status']
        while status in ['In Progress', 'IN_PROGRESS', 'Pending']:
            response = self.get_request(url)
    ...
    def print_validation_errors(self, url):
        self.print_validation_errors_with_response_body(self.get_request(url))

    def print_validation_errors_with_response_body(self, validation_response):
        if "validationChecks" in validation_response:
            ...
            for failed_task in failed_tasks:
                description = failed_task['description'] if 'description' in failed_task else validation_response['description'];
                self.printRed(description + ' ' + 'failed')
    ...
    def user_option_to_enable_vlcm(self, is_vlcm_feature_switch, is_vlcm_enabled):
        if is_vlcm_enabled != True:
            # If vlcm ft is true and user select to enable vlcm images for the cluster then add clusterImageId in the payload
            if is_vlcm_feature_switch == 'true':
                self.printCyan("Lifecycle Management")
                self.printBold("This option will set up clusters in this workload domain with vLCM images and cannot be changed once the workload domain is deployed.")
                vlcm_option = input("\033[1m Do you want to manage clusters in this workload domain using vLCM images?('yes' or 'no'): \033[0m")
                print(*self.two_line_separator, sep='\n')
                regex_yes = 'y(es)?'
                if re.match(regex_yes, vlcm_option, re.IGNORECASE) != None:
                    is_vlcm_enabled = True
        return is_vlcm_enabled
