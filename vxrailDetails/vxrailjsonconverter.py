...
class VxRailJsonConverter:
    def __init__(self, args):
        ...
        self.min_hosts = 3
        self.min_hosts_fc_vlcm = 2
        self.hci_mesh_primary_datastore_name = None
        self.datastore_uuid = []
    ...
    def get_hci_mesh_primary_datastore_name(self):
        return self.hci_mesh_primary_datastore_name

    def get_datastore_uuid(self):
        return self.datastore_uuid

    def get_remote_datastore_uuids(self, domain_id):

        if self.hci_mesh_primary_datastore_name:

            remote_datastores = self.get_remote_datastores(domain_id)
            if self.get_hci_mesh_primary_datastore_name() not in remote_datastores.keys():
                self.utils.printRed("Given datastore '{}' is not present in domain.".format(self.hci_mesh_primary_datastore_name))
                self.utils.printRed("Available datastores in the domain are: {}".format(remote_datastores.keys()))
                exit(1)
            self.datastore_uuid.append(remote_datastores[self.get_hci_mesh_primary_datastore_name()])

    def get_remote_datastores(self, domain_id):
        payload = {'name': 'VSAN_REMOTE_DATASTORES'}
        browse_datastore_url = 'https://{}/v1/domains/{}/datastores/queries'
        browse_datastore_get_url = 'https://{}/v1/domains/{}/datastores/queries/{}'
        workflow_name = "Get remote datastores"
        browse_datastore_post_url = browse_datastore_url.format(self.hostname, domain_id)
        self.utils.get_token()
        response = self.utils.post_request(payload, browse_datastore_post_url)
        self.utils.printGreen('Fetching remote datastore details for given domain with query id: {}'.
                                  format(response['queryInfo']['queryId']))

        browse_datastore_poll_url = browse_datastore_get_url.format(self.hostname, domain_id, response['queryInfo']['queryId'])
        self.utils.printGreen('Polling on get remote datastore api {}'.format(browse_datastore_poll_url))
        time.sleep(10)
        status, result = self.utils.poll_on_queries_for_remote_datastores(browse_datastore_poll_url)
        datastore_names = {}
        if status != 'SUCCEEDED':
            self.utils.printRed('Browse Datastore Failed.')
            self.utils.print_validation_errors(browse_datastore_poll_url)
            exit(1)
        else:
            for element in result['elements']:
                datastore_names.update({element['name']:element['id']})
        return datastore_names

    def get_remote_datastore_details(self):
        cluster_type = self.__get_attr_value(self.vxrail_config, ["global", "cluster_type"])
        if cluster_type == 'DYNAMIC_NODE':
            primary_storage_type = self.__get_attr_value(self.vxrail_config, ["storage", "primary_storage_type"])
            if primary_storage_type is None:
                self.__log_error("Cannot find primary_storage_type field in VxRail JSON")
                self.utils.printRed("Please provide correct storage configuration.")
                exit(1)
            if primary_storage_type == 'VSAN_HCI_MESH':
                vsan_hci_mesh_params = self.__get_attr_value(self.vxrail_config, ["storage", "vsan_hci_mesh_params"])
                if vsan_hci_mesh_params is None:
                    self.__log_error("Cannot find vsan_hci_mesh_params field in VxRail JSON")
                    self.utils.printRed("Invalid vSAN Remote Datastore Detail.")
                    exit(1)
                self.hci_mesh_primary_datastore_name = \
                    self.__get_attr_value \
                        (self.vxrail_config, ["storage", "vsan_hci_mesh_params", "primary_storage_name"])
                if self.hci_mesh_primary_datastore_name is None:
                    self.__log_error("Cannot find hci_mesh_primary_datastore_name field in VxRail JSON")
                    self.utils.printRed("Datastore name is not provided in the storage configuration.")
                    exit(1)

    def parse(self, selected_domain_id, jsonfile, is_primary, enable_vlcm, existing_vcenters_fqdn=None, dvpg_is_on=False):
        if not os.path.exists(jsonfile):
            ...
            try:
                ...
                vxm_payload = self.get_vxm_payload()
                fc_storage = True
                for nw in vxm_payload['networks']:
                    if nw['type'] == 'VSAN':
                        fc_storage = False
                self.get_remote_datastore_details()
                self.__collect_pg_names()
                self.__convert_host_spec(enable_vlcm, fc_storage)
    ...
    def get_vmnics_mapped_to_system_dvs(self, dvpg_is_on, is_mtu_supported):
        ...
        for vdsset in vdssets:
            ...
            if mgmt_is_present != vm_mgmt_is_present and dvpg_is_on:
                print("\033[91m MANAGEMENT and VXRAILSYSTEMVM port groups must be in the same VDS\033[""00m")
                exit(1)
    ...
    def __convert_host_spec(self, enable_vlcm, fc_storage):
        self.host_spec = []
        topdomain = self.__get_attr_value(self.vxrail_config, ["global", "top_level_domain"])
        errors = []
        min_hosts_req = self.min_hosts
        if enable_vlcm and fc_storage:
            min_hosts_req = self.min_hosts_fc_vlcm
        if len(self.__get_attr_value(self.vxrail_config, ["hosts"])) < min_hosts_req:
            errors.append("Please pass {}-node cluster config scenario from VxRail Json input".format(min_hosts_req))
    ...
    def __convert_vxm_payload(self, selected_domain_id, dvpg_is_on):
        ...
        if (cluster_type == 'STANDARD' or cluster_type == 'DYNAMIC_NODE') and vsan_vlan != -1 and vsan_network_present is True:
            vsan_network = {
                "type": "VSAN",
