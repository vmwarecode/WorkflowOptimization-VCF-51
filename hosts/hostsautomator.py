...
class HostsAutomator:
    ...
    def discover_hosts(self, vxrm_fqdn, vxrm_ssl_thumbprint, enable_vlcm, enable_esa):
        ...
        for element in host_discovery_response['elements']:
                if element['status'] == 'UNASSIGNED_USEABLE':
                    ...
                    if enable_esa:
                        if element['isVsanEsaEligible'] == True:
                            discovered_hosts.append(
                                HostDiscovery(element['serialNumber'], element['sshThumbprint'], element['physicalNics'],
                                              element['isPrimary'], softwareInfo))
                        else:
                            self.utils.printRed("The discoverd host {} is not vSAN ESA compatible ".format(element['serialNumber']))
                    else:
                        discovered_hosts.append(
                            HostDiscovery(element['serialNumber'], element['sshThumbprint'], element['physicalNics'],
                                      element['isPrimary'], softwareInfo))

        if not discovered_hosts and enable_esa:
            self.utils.printRed("None of the Host is vSAN ESA Compatible exiting")
            exit(1)
        return discovered_hosts

    def input_hosts_details(self, discovered_hosts, fc_storage, enable_vlcm):
        min_nodes_req = self.min_hosts
        if enable_vlcm and fc_storage:
                min_nodes_req = self.min_hosts_fc_vlcm
        ...
        vlcm_spec = {}
        if enable_vlcm:
            vlcm_spec = self.get_vlcm_spec(discovered_hosts)
        ...
        self.utils.printYellow("** By Default primary node gets selected. Please select atleast {} other node(s).".format(min_nodes_req-1))
        ...
        while True:
            ...
            for host_option in host_options:
                if len(host_option.strip()) == 0:
                    self.utils.printRed("Please select atleast {} other nodes as Minimum {} nodes are required for "
                                        "initial configuration".format(min_nodes_req-1, min_nodes_req))
                    var = False
                if var:
        ...
        if option == "1":
            ...
            for host_fqdn in fqdn_to_serialno.keys():
                hosts_spec.append(self.to_hosts_spec_obj(host_fqdn, password, fqdn_to_serialno[host_fqdn],
                                                         serialno_to_thumbprint[fqdn_to_serialno[host_fqdn]], vlcm_spec))
        else:
            for host_fqdn in fqdn_to_serialno.keys():
    ...
    def get_vlcm_spec(self, discovered_hosts):
        vlcm_spec = {}
        for host in discovered_hosts:
            if host.is_primary:
    ...
    def to_hosts_spec_obj(self, fqdn, password, serial_no, ssh_thumbprint, vlcm_spec):
        host_obj = {
        ...
        if vlcm_spec:
            host_obj["softwareInfo"] = vlcm_spec
        return host_obj


class HostDiscovery:
    def __init__(self, serial_no, ssh_thumbprint, physical_nics, is_primary, softwareInfo):
